...
const UserGrowthChart = () => {
	...
	useEffect(() => {
		axios.get("http://localhost:3000/user-growth")
			.then(response => {
				let data = response.data;

				// If it's { success: true, users: [...] } format
	...
	return (
		...
		>
			<h2 className='text-xl font-semibold mb-4'>Employee Growth (Current Year)</h2>
			<div className='h-[320px]'>
				<ResponsiveContainer width='100%' height='100%'>
					<LineChart data={userGrowthData}>
						...
						<Line
							type='monotone'
							dataKey='users' // Change this if backend key is different, e.g. 'userCount'
							stroke='#8B5CF6'
							strokeWidth={2}
